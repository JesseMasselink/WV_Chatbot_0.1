...
from globals_space import VECTOR_STORE, RETRIEVER
...
Available tools:
- Retrieval Augmented Generation context provider that searches a vector database of the .CSV files, based on the user's input to retrieve contextual information.
...
"""
# TODO: Add to sys prompt when ready
# - PandasAI dataframe reader that is able to generate python query code on the available data .CSV files to provide textual context about the user's input.
...
DATA_FOLDER = pathlib.Path(r"/home/aiadmin/WasteVision/GAD2")
...
LLM_AGENT = create_agent(
    ...
    tools=[retrieve_context]   #TODO: include tools
)
...
def rag():    # Pre-load data or perform any necessary setup here
    ...
    VECTOR_STORE = build_vector_store(chunks, EMBEDDING_MODEL, VECTOR_STORE_PATH)
...
def rag_chatbot_run():
    ...
    if os.path.exists(VECTOR_STORE_PATH):
        ...
        globals_space.VECTOR_STORE = Chroma(
    ...
    globals_space.RETRIEVER = get_retriever(globals_space.VECTOR_STORE, CONTEXT_AMOUNT)
